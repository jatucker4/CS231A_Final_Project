...
def vool_inference(
    ...
    data_pickle_path = "/semantic-abstraction/scene_files/arkit_vn_poster.pkl"
    model_ckpt_path = "/semantic-abstraction/models/ours/vool/vool.pth"
    ...
    for desc, prediction in desc_predictions.items():
        ...
        keep_mask = prediction > prediction.max() - 0.15 # was 0.5
...
@app.command()
def vool_visualize():
    output_path = "/semantic-abstraction/visualization/arkit_vn_poster"
    pointclouds = {
    ...
    rotate = affines.compose(
        T=[0, 0, 0], R=euler.euler2mat(np.pi, 0, 0), Z=[1, 1, 1]
    ) # room
    rotate = affines.compose(
